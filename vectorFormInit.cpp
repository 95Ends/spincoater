...
using namespace std;
   ...
   //STATreq command structure
    ...
    vector <unsigned char> generateVelocityReqCommand (unsigned char& sequenceNumber) {
        vector <unsigned char> command = {
    0xfe,
    0x98, // Protocol ID: 0x98FE
    0x01,       // Protocol version: 0x01
    ...
    //PRDVAlreq command structure
    vector <unsigned char> generatePrdvalReqCommand (unsigned char& sequenceNumber) {
        vector <unsigned char> command = {
    0xfe,
    0x98,       // Protocol ID: 0x98FE
    0x01,       // Protocol version: 0x01
    ...
    */
    ...
    auto velReqCommand = generateVelocityReqCommand(sequenceNumber);
    ...
    auto prdvalReqCommand = generatePrdvalReqCommand(sequenceNumber);

    
    sentBytes = sendto(sockfd, prdvalReqCommand.data(), prdvalReqCommand.size(), 0,
                               (struct sockaddr*)&spinCoaterAddr, sizeof(spinCoaterAddr));
    if (sentBytes < 0) {
